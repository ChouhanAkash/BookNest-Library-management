...
const allBooks = async (req, res) => {
    try {
        // Extract query params from frontend
        let { page = 1, limit = 6, search = "", sort = "title_asc" } = req.query;
        page = parseInt(page);
        limit = parseInt(limit);

        // Search filter (search in title or author, case-insensitive)
        const query = {
            $or: [
        ...
        else if (sort === "year_asc") sortObj = { year: 1 };
...
const returnBook = async (req, res) => {
    try {
        ...
        if (!user.returnedBooks.includes(bookId)) {
