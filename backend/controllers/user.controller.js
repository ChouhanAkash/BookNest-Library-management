    ...
    if (userExist) {
      ...
      if (isPasswordCorrect) {
        const token = jwt.sign(
          ...
          { expiresIn: "1d" }
        ); return res.status(200).json({ message: "user login successfully",token,role:userExist.role})
      }
    }
...
const forgetPassword = async (req, res) => {
  try {
    ...
    const resetLink = `booknestlibraryapp.netlify.app/setNewPassword?token=${resetToken}`;
...
module.exports = { userInfo, userRegister, verifyOtp, userLogin,forgetPassword , allUsers, borrowBook, getBorrowedBooks }